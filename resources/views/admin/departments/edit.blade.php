...
<div class="max-w-xl">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form method="POST"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-4 py-2.5 border rounded-xl text-sm focus:outline-none
                                 ...
                                 {{ $errors->has('description') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">{{ old('description', $department->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{-- Meta info --}}
            <div class="flex gap-4 py-3 px-4 bg-gray-50 rounded-xl text-xs text-gray-400">
                <span>Created: {{ $department->created_at->format('M d, Y') }}</span>
                <span>·</span>
                <span>{{ $department->employees_count ?? $department->employees()->count() }} employee(s)</span>
                <span>·</span>
                <span>{{ $department->spaces_count ?? $department->spaces()->count() }} space(s)</span>
            </div>
